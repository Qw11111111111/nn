...
model = ShallowNet(20, 1, fit_intercept=False)
...
plt.legend(["truth", "model", "model2", "torch"])
plt.grid(True)
plt.show()
...
loss = MSELoss()
optimizer = optim(lr=3e-3, model = model, loss = loss)
...
for epoch in range(EPOCHS):
    ...
    _loss = criterion(predictions, torch.from_numpy(y_train))
...
plt.legend(["moedl1", "model 2", "toech"])
plt.grid(True)
plt.show()
